...
highBoost = Image.open('imagens/lena_gray.bmp')
...
for i in range(largura):
    for j in range(altura):
        mascara.append(round(k*(img.getpixel((i, j)) - imagemSuavizada.getpixel((i, j)))))

indice = 0
for i in range(largura):
    for j in range(altura):
        highBoost.putpixel((i, j), mascara[indice]+img.getpixel((i, j)))
        indice += 1
highBoost.show()
